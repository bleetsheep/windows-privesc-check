...
class Audit(AuditBase):
    ...
    def __init__(self, options, report):
        AuditBase.__init__(self, options)
        self.report = report
    ...
    def run(self):
        ...
        self.run_sub("audit_program_files", self.options.do_all or self.options.do_program_files,
                     self.audit_program_files)
        self.run_sub("audit_registry", self.options.do_all or self.options.do_registry, self.audit_registry)
        self.run_sub("audit_scheduled_tasks", self.options.do_all or self.options.do_scheduled_tasks,
                     self.audit_scheduled_tasks)
        ...
        self.run_sub("audit_installed_software", self.options.do_all or self.options.do_installed_software,
                     self.audit_installed_software)
        self.run_sub("audit_all_files (slow!)",
                     self.options.do_allfiles or self.options.interesting_file_list or self.options.interesting_file_file,
                     self.audit_all_files, self.options)
    ...
    def audit_installed_software(self):
        ...
        for package in packages.get_installed_packages():
            ...
            if package.is_vulnerable_version():
                self.issues.get_by_id('WPC195').add_supporting_data('software_old',
                                                                    [package.get_name(), package.get_publisher(),
                                                                     package.get_version(), package.get_date(),
                                                                     package.get_bad_version()])

            for sw_category in wpc.conf.software.keys():
                if package.is_of_type(sw_category):
                    self.issues.get_by_id(wpc.conf.software[sw_category]['issue']).add_supporting_data('software', [
                        package.get_name(), package.get_publisher(), package.get_version(), package.get_date()])
            ...
            for subkey in eventlogkey.get_subkeys():
                ...
                if filename:
                    ...
                    if filename:
                        ...
                        sd = f.get_sd()
                        if sd:
                            for a in sd.get_acelist().get_untrusted().get_aces_with_perms(
                                    ["FILE_READ_DATA"]).get_aces():
    ...
    def audit_reg_keys(self):
        ...
        for r_user in r_root.get_subkeys():
            ...
            if ss_active and int(ss_active) == 0:
                self.issues.get_by_id("WPC103").add_supporting_data('user_reg_keys',
                                                                    [u, r, "ScreenSaveActive", ss_active])
            ...
            # Screen saver is active
                ...
                if int(ss_secure) > 0:
                    ...
                    if int(ss_timeout) > int(wpc.conf.screensaver_max_timeout_secs):
                        self.issues.get_by_id("WPC091").add_supporting_data('user_reg_keys',
                                                                            [u, r, "ScreenSaveTimeout", ss_timeout])
                else:
                    # should ask for password
                    self.issues.get_by_id("WPC090").add_supporting_data('user_reg_keys',
                                                                        [u, r, "ScreenSaverIsSecure", ss_secure])
        ...
        if defaultuser is not None or password is not None or domain is not None or aal is not None:
            ...
            if aal is None:
                aal = "[not set]"
            self.issues.get_by_id("WPC192").add_supporting_data('aal', [
                'HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon', defaultuser, password,
                domain, aal])
        ...
        if v is not None:
            if v == 0:
                ...
                if v is not None:
                    if v == 0 or v == 5:
                        self.issues.get_by_id("WPC094").add_supporting_data('reg_key_value',
                                                                            [r, "ConsentPromptBehaviorAdmin", v])

                r = RegKey('HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System')
                v = r.get_value("FilterAdministratorToken")
                if v is not None:
                    if v == 0:
                        self.issues.get_by_id("WPC095").add_supporting_data('reg_key_value',
                                                                            [r, "FilterAdministratorToken", v])

                r = RegKey('HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System')
                v = r.get_value("PromptOnSecureDesktop")
                if v is not None:
                    if v == 0:
                        self.issues.get_by_id("WPC097").add_supporting_data('reg_key_value',
                                                                            [r, "PromptOnSecureDesktop", v])
    ...
    def audit_loggedin(self):
        ...
        try:
            while True:
                ...
                for user in users:
                    self.issues.get_by_id("WPC140").add_supporting_data('usernames', [
                        "%s\\%s" % (user['logon_domain'], user['username'])])
                    print("User logged in: Logon Server=\"%s\" Logon Domain=\"%s\" Username=\"%s\"" % (
                    user['logon_server'], user['logon_domain'], user['username']))
                if resume == 0:
                    break
        except:
            print("[E] Failed")
    ...
    def audit_drivers(self):
        ...
        for s in Drivers().get_services():
            if self.options.do_appendices:
                fields = [s.get_name(), s.get_description(), s.get_long_description(), s.get_exe_path_clean()]
                app.add_table_row(fields)
            ...
            if s.get_reg_key() and s.get_reg_key().get_sd():
                ...
                if not s.get_reg_key().get_sd().get_owner().is_trusted():
                    self.issues.get_by_id("WPC142").add_supporting_data('service_exe_regkey_untrusted_ownership',
                                                                        [s, s.get_reg_key()])
                ...
                while parent and parent.get_sd():
                    ...
                    if not parent.get_sd().get_owner().is_trusted():
                        self.issues.get_by_id("WPC148").add_supporting_data('service_regkey_parent_untrusted_ownership',
                                                                            [s, parent])
                    ...
                    if fa_parent:
                        ...
                        if grandparent and grandparent.get_sd():
                            ...
                            fa_grandparent = grandparent.get_issue_acl_for_perms(
                                ["KEY_CREATE_SUB_KEY", "KEY_CREATE_LINK"])
                            if fa_grandparent:
                                self.issues.get_by_id("WPC150").add_supporting_data(
                                    'service_regkey_parent_grandparent_write_perms', [s, fa_parent, fa_grandparent])
            ...
            if str(s.get_exe_path_clean()).find(" ") > 0:  # clean path contains a space
                if str(s.get_exe_path()).find(str(
                        '"' + s.get_exe_path_clean()) + '"') < 0:  # TODO need regexp.  Could get false positive from this.
            ...
            if s.get_exe_file() and s.get_exe_file().get_sd():
                ...
                while parent and parent.get_sd():
                    # Untrusted user owns parent directory
                    if not parent.get_sd().get_owner().is_trusted():
                        self.issues.get_by_id("WPC153").add_supporting_data(
                            'service_exe_parent_dir_untrusted_ownership', [s, parent])
                    ...
                    if fa_parent:
                        ...
                        if grandparent and grandparent.get_sd():
                            ...
                            if fa_grandparent:
                                self.issues.get_by_id("WPC155").add_supporting_data(
                                    'service_exe_parent_grandparent_write_perms', [s, fa_parent, fa_grandparent])
                ...
                if fa:
                    ...
                    if parent and parent.get_sd():
                        ...
                        if fa_parent:
                            self.issues.get_by_id("WPC160").add_supporting_data('service_exe_file_parent_write_perms',
                                                                                [s, fa, fa_parent])
            ...
            if s.get_sd():
                ...
                if not s.get_sd().get_owner().is_trusted():
                    self.issues.get_by_id("WPC163").add_supporting_data('principals_with_service_ownership',
                                                                        [s, s.get_sd().get_owner()])

                # SERVICE_START
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["SERVICE_START"]).get_aces():
                    self.issues.get_by_id("WPC164").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # SERVICE_STOP
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["SERVICE_STOP"]).get_aces():
                    self.issues.get_by_id("WPC165").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # SERVICE_PAUSE_CONTINUE
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(
                        ["SERVICE_PAUSE_CONTINUE"]).get_aces():
                    self.issues.get_by_id("WPC166").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # SERVICE_CHANGE_CONFIG
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(
                        ["SERVICE_CHANGE_CONFIG"]).get_aces():
                    self.issues.get_by_id("WPC167").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # DELETE
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["DELETE"]).get_aces():
                    self.issues.get_by_id("WPC168").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # WRITE_DAC
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["WRITE_DAC"]).get_aces():
                    self.issues.get_by_id("WPC169").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # WRITE_OWNER
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["WRITE_OWNER"]).get_aces():
                    self.issues.get_by_id("WPC170").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])
    ...
    def audit_drives(self):
        ...
        for d in Drives().get_fixed_drives():
            if self.options.do_appendices:
                fields = [d.get_name(), d.get_fs(), d.get_type(), d.is_fixed_drive()]
                app.add_table_row(fields)
            ...
            if d.get_fs() == 'NTFS':
                ...
                for a in directory.get_sd().get_acelist().get_untrusted().get_aces_with_perms(
                        ["FILE_ADD_FILE"]).get_aces():
                    self.issues.get_by_id("WPC010").add_supporting_data('dir_add_file', [directory, a])

                for a in directory.get_sd().get_acelist().get_untrusted().get_aces_with_perms(
                        ["FILE_ADD_SUBDIRECTORY"]).get_aces():
    ...
    def audit_processes(self):
        ...
        for p in Processes().get_all():
            ...
            if self.options.do_appendices:
                fields = [p.get_pid(), p.get_short_name()]
    ...
    def audit_services(self):
        ...
        for s in Services().get_services():
            ...
            if s.get_reg_key() and s.get_reg_key().get_sd():
                ...
                # Check owner
                if not s.get_reg_key().get_sd().get_owner().is_trusted():
                    self.issues.get_by_id("WPC035").add_supporting_data('service_exe_regkey_untrusted_ownership',
                                                                        [s, s.get_reg_key()])
                ...
                while parent and parent.get_sd():
                    # Untrusted user owns parent directory
                    if not parent.get_sd().get_owner().is_trusted():
                        self.issues.get_by_id("WPC041").add_supporting_data('service_regkey_parent_untrusted_ownership',
                                                                            [s, parent])
                    ...
                    if fa_parent:
                        ...
                        if grandparent and grandparent.get_sd():
                            ...
                            fa_grandparent = grandparent.get_issue_acl_for_perms(
                                ["KEY_CREATE_SUB_KEY", "KEY_CREATE_LINK"])
                            if fa_grandparent:
                                self.issues.get_by_id("WPC043").add_supporting_data(
                                    'service_regkey_parent_grandparent_write_perms', [s, fa_parent, fa_grandparent])
            ...
            if s.get_exe_file() and s.get_exe_file().get_sd():
                ...
                while parent and parent.get_sd():
                    # Untrusted user owns parent directory
                    if not parent.get_sd().get_owner().is_trusted():
                        self.issues.get_by_id("WPC033").add_supporting_data(
                            'service_exe_parent_dir_untrusted_ownership', [s, parent])
                    ...
                    if fa_parent:
                        ...
                        if grandparent and grandparent.get_sd():
                            fa_grandparent = grandparent.get_file_acl_for_perms(["FILE_ADD_SUBFOLDER"])
                            if fa_grandparent:
                                self.issues.get_by_id("WPC031").add_supporting_data(
                                    'service_exe_parent_grandparent_write_perms', [s, fa_parent, fa_grandparent])
                ...
                if fa:
                    ...
                    if parent and parent.get_sd():
                        fa_parent = parent.get_file_acl_for_perms(["FILE_ADD_FILE"])
                        if fa_parent:
                            self.issues.get_by_id("WPC034").add_supporting_data('service_exe_file_parent_write_perms',
                                                                                [s, fa, fa_parent])
            ...
            if s.get_sd():
                ...
                # TODO all mine are owned by SYSTEM.  Maybe this issue can never occur!?
                if not s.get_sd().get_owner().is_trusted():
                    self.issues.get_by_id("WPC025").add_supporting_data('principals_with_service_ownership',
                                                                        [s, s.get_sd().get_owner()])

                # SERVICE_START
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["SERVICE_START"]).get_aces():
                    self.issues.get_by_id("WPC018").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # SERVICE_STOP
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["SERVICE_STOP"]).get_aces():
                    self.issues.get_by_id("WPC019").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # SERVICE_PAUSE_CONTINUE
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(
                        ["SERVICE_PAUSE_CONTINUE"]).get_aces():
                    self.issues.get_by_id("WPC020").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # SERVICE_CHANGE_CONFIG
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(
                        ["SERVICE_CHANGE_CONFIG"]).get_aces():
                    self.issues.get_by_id("WPC021").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # DELETE
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["DELETE"]).get_aces():
                    self.issues.get_by_id("WPC022").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # WRITE_DAC
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["WRITE_DAC"]).get_aces():
                    self.issues.get_by_id("WPC023").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])

                # WRITE_OWNER
                for a in s.get_sd().get_acelist().get_untrusted().get_aces_with_perms(["WRITE_OWNER"]).get_aces():
                    self.issues.get_by_id("WPC024").add_supporting_data('principals_with_service_perm',
                                                                        [s, a.get_principal()])
    ...
    def audit_scheduled_tasks(self):
        ...
        for task in ScheduledTasks().get_all_tasks():
            if self.options.do_appendices:
                fields = [task.get_name(), task.get_description(),
                          task.get_enabled(), task.get_sd_text(),
                          task.get_action_context(), task.get_command_path()]
                app.add_table_row(fields)
            ...
            if 1 and task.get_command_path():
                ...
                if f.is_replaceable():
                    ...
                    for a in f.get_dangerous_aces():
                        self.issues.get_by_id("WPC120").add_supporting_data('scheduled_task_exe_perms',
                                                                            [f.get_name(), f, a])
                    ...
                    if clsid:
                        ...
                        for reg_val_file in reg_val_files:
                            ...
                            if f and f.is_replaceable():
                                ...
                                self.issues.get_by_id(check_id).add_supporting_data('regkey_ref_replacable_file',
                                                                                    [check_type, name, clsid, f, s])
                    ...
                    for v in r.get_values():
                        if wpc.utils.looks_like_executable(r.get_value(v)):
                            for a in set_value_aces:
                                self.issues.get_by_id("WPC115").add_supporting_data('regkey_value_data_perms',
                                                                                    [r, v, repr(r.get_value(v)), a])
                        if wpc.utils.looks_like_path(r.get_value(v)):
                            for a in set_value_aces:
                                self.issues.get_by_id("WPC116").add_supporting_data('regkey_value_data_perms',
                                                                                    [r, v, repr(r.get_value(v)), a])
                        if wpc.utils.looks_like_registry_path(r.get_value(v)):
                            for a in set_value_aces:
                                self.issues.get_by_id("WPC117").add_supporting_data('regkey_value_data_perms',
                                                                                    [r, v, repr(r.get_value(v)), a])
                        if wpc.utils.looks_like_ip_address(r.get_value(v)):
                            for a in set_value_aces:
                                self.issues.get_by_id("WPC118").add_supporting_data('regkey_value_data_perms',
                                                                                    [r, v, repr(r.get_value(v)), a])
                        if wpc.utils.looks_like_user(r.get_value(v)):
                            for a in set_value_aces:
                                self.issues.get_by_id("WPC119").add_supporting_data('regkey_value_data_perms',
                                                                                    [r, v, repr(r.get_value(v)), a])
                        ...
                        try:
                            ...
                            for line in open(filename, 'r'):
                                ...
                                if re.search(check['filename_content_regex'], line, re.IGNORECASE):
                                    self.issues.get_by_id(check['issue']).add_supporting_data('filename_string',
                                                                                              [filename])
                                    break
                        except:
                            pass
