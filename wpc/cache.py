...
class Cache:
    ...
    def LsaOpenPolicy(self, server, rights):
        ...
        if not keystring in self.policyhandlefromserverrights.keys():
            self.policyhandlefromserverrights[keystring] = win32security.LsaOpenPolicy(wpc.conf.remote_server,
                                                                                       win32security.POLICY_VIEW_LOCAL_INFORMATION | win32security.POLICY_LOOKUP_NAMES)
    ...
    def NetGroupGetUsers(self, server, name, level):
        ...
        while keepgoing:
            try:
                m, total, resume = win32net.NetGroupGetUsers(server, name, level, resume,
    ...
    def NetLocalGroupGetMembers(self, server, name, level):
        ...
        while keepgoing:
            try:
                m, total, resume = win32net.NetLocalGroupGetMembers(server, name, level, resume,
                                                                    win32netcon.MAX_PREFERRED_LENGTH)
            except:
                return []
